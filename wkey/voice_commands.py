...
# TTS implementation lives in tts_engine.py (Phase 6 split). Re-exported here
# so every existing consumer (faster_whisper_Mother_of_all_wkey.py's
# voice_commands_module.TTS_queue, tests, conftest stubs) keeps working
# unchanged. Importing this triggers tts_engine's worker threads to start,
...
try:
    from .tts_engine import (
        ...
        process_TTS_queue,
        process_TTS_Audio_play_queue,
        text_to_speech,
        fallback_kokoro_tts,
        fallback_offline_tts,
        DEFAULT_TTS_SPEED,
        DEFAULT_EDGE_VOICE,
        EDGE_TTS_FALLBACK_VOICES,
        KOKORO_FALLBACK_VOICES,
        ENABLE_PYTTS_FALLBACK,
    )
