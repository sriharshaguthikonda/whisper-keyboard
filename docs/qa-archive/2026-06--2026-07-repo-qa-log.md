...
## 2026-07-12 Rust Broker Retirement — PLANNING ONLY

Status: planning. No code changes this session. I confirm direction with you + ChatGPT, then a separate implementation agent does the work.

### Confirmed root cause (verified in code + your 2026-07-07 log)

The Rust hook itself is probably NOT what dies. The death is the 5-layer chain fighting itself on wake:

`Scheduler(wake-kill) -> Start-WKeyBroker.bat -> PowerShell supervisor -> Rust broker -> Python -> runtime lock`

Exact wake failure (your log around Q&A lines 1003-1049):
...
4. Python sees a stale `wkey_runtime.lock (pid=unknown)` still held -> "Another Whisper Keyboard backend is already running. Exiting."
5. Broker event channel disconnects -> "python event channel disconnected while child was still running" -> exit 1.
6. PowerShell retries with backoff 5 times, then gives up. Dictation is dead until you restart.

Three code-level faults behind it:
- `scripts/Install-WKeyBrokerTask.ps1`: `StopExisting` + `StopIfGoingOnBatteries=true` + a wake EventTrigger (Power-Troubleshooter id 1) alongside the logon trigger. So wake = kill-and-restart BY DESIGN, and going on battery = stop by design (wrong for a ThinkPad). This is the scheduled-task change you made on 2026-06-15 and asked me to check — it is the trigger.
- `native/wkey-broker/src/main.rs:292`: `engine.recv_event_timeout(Duration::from_secs(5))?` — any slow Python reply (mic reopen, volume op, lock wait) kills the broker.
- The Rust "supervisor" never actually restarts Python. When Python exits it just errors and the broker dies; PowerShell (5 retries) is the real supervisor.

Stale-lock race = proximate killer. Wake-kill = what keeps re-triggering it.

### Direction (ChatGPT's verdict + my read)

Don't repair the 5-layer design. Go **[external remapper] -> F23/F24 -> Python**. Python already has a pynput listener that can catch F23/F24. That deletes the stdio protocol, the 5s timeout, the job object, the PowerShell restart loop, competing lock ownership, and most duplicate-lock races.

I agree. The broker's only genuinely-valuable job was low-level suppression of letter-chords (like D+F) while typing. If the trigger is a clean function key, you don't need a suppressing hook at all — pynput catching a clean synthetic key is reliable.

### Options

| Opt | What | Cost | My take |
|---|---|---|---|
| A | Delete broker. External remapper (Kanata or PowerToys) emits F23/F24; Python catches them. Logon-only task, no wake-kill. | Lowest code. One external dep you maintain. | Recommended IF a remapper already gives you F23/F24 reliably |
| B | Keep a TINY rust input-only bridge: hook -> SendInput F23/F24. No python ownership, no stdio, no supervision. | Keeps a rust binary + a 2nd process to start. No external dep. | Only if you want zero external tools |
| C | Keep broker, fix the 3 faults. | Most code + most fragility retained. | Not recommended (ChatGPT agrees) |

Regardless of A/B/C: fix the scheduler to **logon-only, IgnoreNew, no battery-stop, no wake trigger**, and move sleep/resume health into the app behind a toggle — exactly what you asked for on 2026-07-07 ("inbuilt, behind a toggle, not this complicated bat and scheduler").

### 3 questions that change the plan (answer inline; I keep working)

**Q1 — Kanata reality.** ChatGPT assumes you already have Kanata + the Interception driver installed and working with F23/F24 defined. I found ZERO trace of Kanata anywhere in the repo. Do you actually have Kanata+Interception installed and running now? Or is your remapping today only PowerToys?

**Q2 — Trigger.** Are clean function keys enough as the permanent triggers — F23 = dictation/paste, F24 = command/tool-use — produced by PowerToys/Kanata? Or do you still want a letter-chord like D+F (the only thing that truly needs a low-level suppressing hook)?

**Q3 — Git.** To retire the broker I recommend **abandon-in-place**: one forward commit that deletes `native/` and stops wiring the broker, keeping all history. Your "rewind to before the broker and cherry-pick the good commits" idea is riskier here: the broker commits are interleaved with Control-Center, speaker-filter and Ask-AI commits since `03edf96`, and Control-Center/activation code references broker mode. Same end state, far less risk. OK with abandon-in-place, or do you specifically want the clean rewound history?

### Meanwhile
- Sent ChatGPT a fresh discussion prompt via the model bridge (job `20260712T025046Z_e3de60a94011d4fb`). Fetch side has been flaky; if it doesn't return automatically I'll ask you to paste its reply.
- Continuing now on unblocked planning: exact broker-vs-non-broker commit split, then draft design spec + roadmap linkage. I will NOT finalize/commit docs until you pick A/B and answer Q1-Q3.

### Progress (drafts written, not committed)

- ChatGPT bridge fetch came back with only citation fragments ("Verified `feature / Microsoft Learn / GitHub"), so the scrape side is still broken. The prompt DID reach your tab — paste its reply here when you can, though the plan already folds in its saved verdict.
- Commit entanglement mapped: since `03edf96`, pure-broker ≈ 4, mixed ≈ 6, plus broker wiring in shared launcher scripts + main-runtime broker branches. Broker + Control-Center/speaker-filter/Ask-AI commits are interleaved → clean rewind is messy. Retirement surface in the current tree is bounded (native/ crate, broker_control.py, 30 flag-gated refs in the main runtime, 1 control-center ref, 3 launcher scripts).
- Drafted (planning only, uncommitted):
  - `docs/superpowers/specs/2026-07-12-input-owner-simplification-design.md`
  - `docs/superpowers/plans/2026-07-12-broker-retirement-git-strategy.md`
  - ROADMAP.md: broker section marked SUPERSEDED + new "Input-Owner Simplification" section + your 2026-07-07 future UX items (cursor visual feedback, settings search palette) linked in.
- Waiting on Q1 (Kanata installed?), Q2 (F-keys enough vs D+F chord?), Q3 (abandon-in-place vs rewind?). Once you answer, I finalize the per-commit phase plan and hand off to the implementation agent.


## user comments
1. kanata is installed and it should be there somewhere in ai tools or tools in the c folder. and it is also running vd toggle kbd sort of config file. you can get more details from that.
2. df trigger f23 and then through that it goes to the app so on and so forth in kanata and f24 for the command line tool. the original key mapping is tilda.
3. keep the plan  files in the repo don't keep it somewhere else and link them properly.
4. this file is becoming large. you need to move the unnecessary things into archivemd and then keep only the necessary things and the topic of discussion in here. if there are any other pre-existing issues as well or points that have not been addressed, keep them here.
5. github , i think the best strategy is, i don't know what you are talking about, all the technicalities, but we do want the history in the sense what we have done and we fucked it up or like you know the problems persisted or it got only complicated. all that information stays in the git history and yeah, that's the general idea.
6. other than that, you can do what is best.

7. this ssd should work in different production environments, laptop, desktop, new hardware, old hardware.
8 .the complication is that this shb moves between different systems.

## 2026-07-12 Phase 1 implementation status

Implementing repo-local direct Python startup only: scheduled task is logon-only, IgnoreNew, battery-safe, and RestartOnFailure; malformed/unknown runtime-lock ownership gets one bounded retry. Existing Ask-AI work is untouched. No commit/push and no live scheduled-task changes.
